...
| 10    | Production polish                   | lint, format and 2173 tests, run on CI; preferences persist                         |
...
| **A file on disk**                         | done  | A UFO folder is opened, saved back to, and remembered for next time, through the File System Access API. Saving is manual: the working store autosaves, and a folder the user chose is somewhere the editor is a guest. Only the files that differ from what the last save left are written, and the status bar counts them as they go.                                                                                             |
